$(function() {
    ...
    //时间选择控制
    setDateMinNow($("#start-time"));
    setDateMinNow($("#end-time"), 1);
    $("#start-time").on("change", function() {
        $("#span-start-time").html($(this).val());
        changeListen();
    });
    $("#end-time").on("change", function() {
        $("#span-end-time").html($(this).val());
        changeListen();
    });
...
function changeListen() {
    ...
    if (parseFloat($_pay.val()) > 10 && parseFloat($_stay.val()) > 10 && $_img_gray.length < 1 && checkDate()) {
